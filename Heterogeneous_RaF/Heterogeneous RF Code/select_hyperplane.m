function [bestCutVar,W_new,gini,min_impurity]=select_hyperplane(W,Data,Labels,minleaf,gini_ldampsvm,numOfpartitions_ldampsvm)
...
numofBound = size(W,2);
numofBound_rest = numofBound-numOfpartitions_ldampsvm;
W_temp = W(:,numOfpartitions_ldampsvm+1:end);

gini_rest = ones(1,numofBound_rest);
...
for z=1:numofBound_rest %loop over all decision boundaries
    ...
    ratio2 = 1-ratio1;
    gini_rest(1,z) = ratio1*(1-sum(count_pos))+ratio2*(1-sum(count_neg));
end

if numOfpartitions_ldampsvm>0
    gini = [gini_ldampsvm gini_rest];
else
    gini = gini_rest;
end
