function [bestCutVar,W_new]=select_hyperplane_psvm(W,Data,Labels,minleaf)
...
if W(:,1)~=W(:,2)
    ...
    W4=W(:,1);
     ...
     Y1=Data*W3(1:end-1);
     Y2=Data*W4(1:end-1);
     index_pos=find(Y1>W3(end));
     index_neg=find(Y1<=W3(end));
   ...
   gini1=ratio1*(1-sum(count_pos))+ratio2*(1-sum(count_neg));  
     ...
     index_pos=find(Y2>W4(end));
     index_neg=find(Y2<=W4(end));
     label_pos=Labels(index_pos);
     label_neg=Labels(index_neg);
