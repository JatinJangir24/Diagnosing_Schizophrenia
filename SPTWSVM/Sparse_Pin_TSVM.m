...
function [accuracy, non_zero_dual_variables, training_time, lambda, Y_Predicted] = Sparse_Pin_TSVM(X1_Train, X2_Train, X_Test, Y_Test, c, epsilon, tau)
    ...
    Q = [G * inv(H' * H + delta .* eye(n + 1)) * G' zeros(m2, m2); zeros(m2, m2) zeros(m2, m2)];
    ...
    u = -inv(H' * H + delta .* eye(n + 1)) * G' * lambda;
    ...
    u = inv(G' * G + delta .* eye(n + 1)) * H' * lambda;
