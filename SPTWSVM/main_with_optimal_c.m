...
for i = 1: size(epsilon, 1)
	...
	for j = 1: size(tau, 1)
		...
		for k = 1: 11
			c = c*10;
			[accuracy, non_zero_dual_variables, training_time, lambda] = Sparse_Pin_TSVM(X1_Train, X2_Train, X_Test, Y_Test, c, epsilon(i), tau(j));
			if(maxx < accuracy)
				maxx = accuracy;
				finalc = c;
			end
...
% 		for k = 1: 11
% 			c = c*10;
% 			[accuracy, non_zero_dual_variables, training_time, lambda] = Sparse_Pin_SVM(X1_Train, X2_Train, Y_Train, X_Test, Y_Test, c, epsilon(i), tau(j));
% 			if(maxx < accuracy)
% 				maxx = accuracy;
% 				finalc = c;
% 			end
