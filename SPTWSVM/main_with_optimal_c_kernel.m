...
for i = 1: size(epsilon, 1)
	...
	for j = 1: size(tau, 1)
		...
		for k = 1: 11
			...
			for l = 1: 11
				gamma = gamma*10;
				[accuracy, non_zero_dual_variables, training_time, lambda] = Sparse_Pin_TSVM_Kernel(X1_Train, X2_Train, X_Test, Y_Test, c, gamma, epsilon(i), tau(j));
				if(maxx < accuracy)
					maxx = accuracy;
					finalc = c;
					finalgamma = gamma;
		...
		temp = [temp accuracy];
		tempc = [tempc finalc];
		tempgamma = [tempgamma finalgamma];
		if(tau(j) == 0.5)
...
% 			for l = 1: 11
% 				gamma = gamma*10;
% 				[accuracy, non_zero_dual_variables, training_time, lambda] = Sparse_Pin_SVM_Kernel(X1_Train, X2_Train, Y_Train, X_Test, Y_Test, c, gamma, epsilon(i), tau(j));
% 				if(maxx < accuracy)
% 					maxx = accuracy;
% 					finalc = c;
% 					finalgamma = gamma;
