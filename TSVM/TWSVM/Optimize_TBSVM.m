function OptPara=Optimize_TBSVM(validation_Index)
...
for c1 =Range_C1 							%Iterate on 13 values of c1
    for c3=Range_C1
        for gamma = Range_gamma     %Iterate on 13 values of c3
            ...
            FunPara=struct('c1',c1,'c3',c3,'kerfPara',struct('type',kern.type,'pars',gamma));
            ...
            for va = 1:p                                  %# for each fold
                ...
                [Predict_Y,train_tym]=TWSVM([testX, testY],[trainX, trainY],FunPara);
    ...
    disp(['TBSVM: c1=', num2str(c1),' Gamma= ', num2str(gamma),' Accuracy= ',num2str(temp_AUC)]);
end %for K i.e. C1
disp('Completed')
